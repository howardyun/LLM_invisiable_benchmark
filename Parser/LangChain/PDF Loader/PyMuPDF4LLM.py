...
def write_docs_to_file(pdf_file_path: Path, final_output_path: Path, docs) -> None:
    ...
    with final_output_path.open("w", encoding="utf-8") as file_obj:
        header = (
            f"文件解析结果 (PyMuPDF4LLM): {pdf_file_path.name}\n"
...
def load_pdf_with_pymupdf4llm(input_path: str | Path, output_path: str | Path) -> float:
    ...
    docs = PyMuPDF4LLMLoader(str(pdf_file_path)).load()
...
def load_pdf_with_pymupdf4llm_batch(input_dir: str | Path, output_root: str | Path, output_name: str, recursive: bool = True) -> None:
    ...
    for index, pdf_path in enumerate(pdf_files, start=1):
        ...
        try:
            elapsed_seconds = load_pdf_with_pymupdf4llm(pdf_path, output_path)
            success_count += 1
...
def main() -> None:
    ...
    parser.add_argument("-i", "--input", type=str, default="../text_version.pdf", help="输入 PDF 文件路径，或批处理目录路径")
    parser.add_argument("-o", "--output", type=str, default="Output/PyMuPDF4LLMLoader.txt", help="单文件模式输出 txt 路径；目录模式输出根目录")
    parser.add_argument("--output-name", type=str, default="PyMuPDF4LLMLoader.txt", help="目录批处理模式下，每个 PDF 的输出文件名")
    parser.add_argument("--recursive", action=argparse.BooleanOptionalAction, default=True, help="目录模式下是否递归扫描 PDF，默认递归")
    args = parser.parse_args()

    input_path = Path(args.input).resolve()
    if input_path.is_dir():
        load_pdf_with_pymupdf4llm_batch(input_path, args.output, args.output_name, recursive=args.recursive)
    else:
