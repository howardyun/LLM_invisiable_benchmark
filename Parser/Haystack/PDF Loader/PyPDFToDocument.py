...
def load_pdf_haystack(input_path, output_path):
    # 处理路径（转换为绝对路径）
    ...
    try:
        # 初始化 Haystack Converter 并加载文档
        print("正在初始化 Haystack PyPDFToDocument 并解析 PDF，请稍候...")

        converter = PyPDFToDocument()

        # Haystack 的 run 方法接受 sources 列表 (支持 Path 对象或字符串)
        # 返回结果是一个字典，键为 "documents"
        ...
        with open(final_output_path, "w", encoding="utf-8") as f:
            ...
            header = f"文件解析结果 (Haystack PyPDFToDocument): {os.path.basename(pdf_file_path)}\n" \
            ...
            for i, doc in enumerate(docs):
                ...
                # --- 提取并格式化元数据 ---
                # Haystack Document 对象使用 .meta 属性 (LangChain 是 .metadata)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="Output/Haystack_PyPDF.txt",
        help="输出 txt 文件的路径 (默认: Output/Haystack_PyPDF.txt)"
    )
