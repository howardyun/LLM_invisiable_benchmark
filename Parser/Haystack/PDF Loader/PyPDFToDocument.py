...
def write_docs_to_file(pdf_file_path: Path, output_path: Path, docs) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as file_obj:
        header = (
            f"文件解析结果 (Haystack PyPDFToDocument): {pdf_file_path.name}\n"
            f"保存位置: {output_path}\n"
            f"文档片段数: {len(docs)}\n"
            f"{'=' * 30}\n\n"
        )
...
def load_pdf_haystack(input_path: str | Path, output_path: str | Path, converter: PyPDFToDocument | None = None) -> float:
    ...
    if converter is None:
        print("正在初始化 Haystack PyPDFToDocument 并解析 PDF，请稍候...")
...
def load_pdf_haystack_batch(input_dir: str | Path, output_root: str | Path, output_name: str, recursive: bool = True) -> None:
    ...
    print("正在初始化 Haystack PyPDFToDocument（批处理模式，仅初始化一次）...")
    converter = PyPDFToDocument()
    total = len(pdf_files)
    success_count = 0
    failed_count = 0
    ...
    for index, pdf_path in enumerate(pdf_files, start=1):
        ...
        try:
            elapsed_seconds = load_pdf_haystack(pdf_path, output_path, converter=converter)
            success_count += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Haystack PyPDFToDocument 解析工具")
    parser.add_argument("-i", "--input", type=str, default="../../PDF/Zero_Width/text/zero_width.pdf", help="输入 PDF 文件路径，或批处理目录路径")
    parser.add_argument("-o", "--output", type=str, default="Output/Haystack_PyPDF.txt", help="单文件模式输出 txt 路径；目录模式输出根目录")
    parser.add_argument("--output-name", type=str, default="PyPDFToDocument.txt", help="目录批处理模式下，每个 PDF 的输出文件名")
    parser.add_argument("--recursive", action=argparse.BooleanOptionalAction, default=True, help="目录模式下是否递归扫描 PDF，默认递归")
    args = parser.parse_args()

    input_path = Path(args.input).resolve()
    if input_path.is_dir():
        load_pdf_haystack_batch(input_path, args.output, args.output_name, recursive=args.recursive)
    else:
        load_pdf_haystack(args.input, args.output)
