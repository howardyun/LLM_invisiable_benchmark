...
def write_docs_to_file(pdf_file_path: Path, output_path: Path, docs) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as file_obj:
        header = (
            f"文件解析结果 (PDFMarkerReader): {pdf_file_path.name}\n"
            f"保存位置: {output_path}\n"
...
def load_pdf_marker(input_path: str | Path, output_path: str | Path, loader: PDFMarkerReader | None = None) -> float:
    ...
    if not pdf_file_path.exists():
        ...
        return 0.0
    if loader is None:
        print("正在初始化 PDFMarkerReader...")
        print("注意: 首次运行可能会自动下载模型权重，请保持网络通畅。")
...
def load_pdf_marker_batch(input_dir: str | Path, output_root: str | Path, output_name: str, recursive: bool = True) -> None:
    ...
    if not pdf_files:
        ...
        return
    print("正在初始化 PDFMarkerReader（批处理模式，仅初始化一次）...")
    loader = PDFMarkerReader()
    total = len(pdf_files)
    success_count = 0
    failed_count = 0
    ...
    for index, pdf_path in enumerate(pdf_files, start=1):
        ...
        try:
            elapsed_seconds = load_pdf_marker(pdf_path, output_path, loader=loader)
            success_count += 1
...
def main() -> None:
    ...
    parser.add_argument("-i", "--input", type=str, default="../../PDF/Word_Shift/pdf_poc.pdf", help="输入 PDF 路径，或批处理目录路径")
    parser.add_argument("-o", "--output", type=str, default="Output/word_shift/PDFMarkerLoader.txt", help="单文件模式输出 txt 路径；目录模式输出根目录")
    parser.add_argument("--output-name", type=str, default="PDFMarkerLoader.txt", help="目录批处理模式下，每个 PDF 的输出文件名")
    parser.add_argument("--recursive", action=argparse.BooleanOptionalAction, default=True, help="目录模式下是否递归扫描 PDF，默认递归")
    args = parser.parse_args()
    input_path = Path(args.input).resolve()
    if input_path.is_dir():
        load_pdf_marker_batch(input_path, args.output, args.output_name, recursive=args.recursive)
    else:
