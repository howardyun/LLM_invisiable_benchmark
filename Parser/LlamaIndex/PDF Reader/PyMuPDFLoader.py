...
def write_docs_to_file(pdf_file_path: Path, output_path: Path, docs) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as file_obj:
        header = (
            f"文件解析结果 (PyMuPDFReader): {pdf_file_path.name}\n"
            f"保存位置: {output_path}\n"
            f"解析出的页面数量: {len(docs)}\n"
...
def load_pdf_pymu(input_path: str | Path, output_path: str | Path, loader: PyMuPDFReader | None = None) -> float:
...
def load_pdf_pymu_batch(input_dir: str | Path, output_root: str | Path, output_name: str, recursive: bool = True) -> None:
    ...
    if not pdf_files:
        ...
        return
    print("正在初始化 PyMuPDFReader（批处理模式，仅初始化一次）...")
    loader = PyMuPDFReader()
    total = len(pdf_files)
    success_count = 0
    failed_count = 0
    ...
    for index, pdf_path in enumerate(pdf_files, start=1):
        ...
        try:
            elapsed_seconds = load_pdf_pymu(pdf_path, output_path, loader=loader)
            success_count += 1
...
def main() -> None:
    ...
    if input_path.is_dir():
        load_pdf_pymu_batch(input_path, args.output, args.output_name, recursive=args.recursive)
    else:
